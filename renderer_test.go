...
func TestRenderNarrativeMarkdownUsesStableActionableStructure(t *testing.T) {
	...
	wants := []string{
		...
		"The required policy is not assigned, applied, or scoped",
		"possible diagnostic explanations, not confirmed causes",
...
func TestRenderZeroComplianceSectionListsEverySignalWithoutGeneratedGuidance(t *testing.T) {
	...
	for _, signalID := range []string{"zero_a", "zero_b", "zero_c", "zero_d", "zero_e", "zero_f"} {
		if got := strings.Count(markdown, "`"+signalID+"`"); got != 1 {
			t.Fatalf("signal ID %q appears %d times, want once:\n%s", signalID, got, markdown)
		}
	}
	if got := strings.Count(markdown, "#### Common Reasons to Investigate"); got != 1 {
		t.Fatalf("common-reasons heading appears %d times, want once:\n%s", got, markdown)
	}
	for _, unwanted := range []string{"Remediation Theme", "**Why it matters:**", "##### Implementation Impact"} {
		if strings.Contains(markdown, unwanted) {
			t.Fatalf("zero-only report contains generated zero guidance %q:\n%s", unwanted, markdown)
		}
	}
}

func TestRenderNarrativeMarkdownOmitsEmptyRemediationSequence(t *testing.T) {
	report := &shared.CIDReport{Platforms: []shared.PlatformSummary{{
		Name:       "Windows 11",
		Compliance: shared.ComplianceMap{"independent_control": 0.5},
	}}}
	analysis, err := AnalyzeReport(report)
	if err != nil {
		t.Fatalf("AnalyzeReport returned error: %v", err)
	...
	guidance := validGuidanceForAnalysis(analysis)
	guidance.Platforms[0].RemediationSequence = []string{}

	markdown, err := RenderNarrativeMarkdown(analysis, guidance)
	if err != nil {
		t.Fatalf("RenderNarrativeMarkdown returned error: %v", err)
	}
	if strings.Contains(markdown, "### Recommended Remediation Sequence") {
		t.Fatalf("markdown contains an empty remediation-sequence section:\n%s", markdown)
	}
	if !strings.Contains(markdown, "##### Remediation") {
		t.Fatalf("markdown lost the finding-level remediation:\n%s", markdown)
	}
}
