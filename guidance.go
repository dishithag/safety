...
const (
	maxGroupedZeroGuidance = 5
	maxAdminTerminology    = 3
)
...
func ParseNarrativeGuidance(raw string, analysis *ReportAnalysis) (*NarrativeGuidance, error) {
	...
	normalizeNarrativeGuidance(&guidance, analysis)
...
func normalizeNarrativeGuidance(guidance *NarrativeGuidance, analysis *ReportAnalysis) {
	platformCount := min(len(guidance.Platforms), len(analysis.Platforms))
	for i := range platformCount {
		if analysis.Platforms[i].ZeroGuidanceMode != "grouped" {
			continue
		}
		guidance.Platforms[i].ZeroGroups = normalizeGroupedZeroGuidance(guidance.Platforms[i].ZeroGroups)
	}
}

func normalizeGroupedZeroGuidance(groups []ZeroGroupGuidance) []ZeroGroupGuidance {
	normalized := make([]ZeroGroupGuidance, 0, len(groups))
