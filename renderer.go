...
func renderHighLevelOverview(b *strings.Builder, analysis *ReportAnalysis) {
	...
	if len(analysis.Platforms) == 0 {
		fmt.Fprintln(b, "**Data note:** The source report contains no platform-level results.")
	}
}
...
func renderPlatform(b *strings.Builder, index int, platform *PlatformAnalysis, guidance *PlatformGuidance, mode narrativeMode, showReportedDevices, showScoreSummary bool) {
	...
	if len(platform.ZeroSignals) > 0 {
		renderZeroComplianceSection(b, platform)
	}
	if len(platform.PrioritySignals) > 0 {
		...
		fmt.Fprintln(b)
		fmt.Fprintln(b, "No partially compliant signals remain after the zero-compliance gaps; investigate the possible causes listed in that section before making configuration changes.")
	}
...
func renderZeroComplianceSection(b *strings.Builder, platform *PlatformAnalysis) {
	fmt.Fprintln(b)
	...
	fmt.Fprintln(b)
	fmt.Fprintln(b, "#### Common Reasons to Investigate")
	fmt.Fprintln(b)
	fmt.Fprintln(b, "- A control is not configured or is not currently active.")
	fmt.Fprintln(b, "- The required policy is not assigned, applied, or scoped to the represented devices.")
	fmt.Fprintln(b, "- A hardware, firmware, operating-system, or dependent-control prerequisite is unavailable.")
	fmt.Fprintln(b, "- A required reboot, device check-in, or policy refresh has not completed.")
	fmt.Fprintln(b, "- A control is unsupported or not applicable to the represented device population.")
	fmt.Fprintln(b, "- Sensor, inventory, or assessment telemetry is missing, stale, or incomplete.")
	fmt.Fprintln(b)
	fmt.Fprintln(b, "> These are possible diagnostic explanations, not confirmed causes. Validate applicability and representative devices before making configuration changes.")
}
...
func renderTechnicalGuidance(b *strings.Builder, guidance *TechnicalGuidance) {
	...
	fmt.Fprintln(b)
	fmt.Fprintln(b, "##### Implementation Impact")
	fmt.Fprintln(b)
	fmt.Fprintf(
		b,
