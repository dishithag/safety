...
func TestBatchCompletionErrorReportsPartialFailure(t *testing.T) {
	if err := batchCompletionError(runStats{Total: 4, Processed: 4}); err != nil {
		t.Fatalf("successful batch returned error: %v", err)
	}

	err := batchCompletionError(runStats{Total: 4, Processed: 3, Failed: 1})
	if err == nil || err.Error() != "1 of 4 cid reports failed" {
		t.Fatalf("partial batch error = %v, want failed-report tally", err)
	}

	err = batchCompletionError(runStats{Total: 4, Processed: 3, Fallback: 1})
	if err == nil || err.Error() != "1 of 4 cid reports used deterministic fallback" {
		t.Fatalf("degraded batch error = %v, want fallback tally", err)
	}
}
...
func TestRunWritesFallbackAndRetriesItOnNextGenAIRun(t *testing.T) {
	profile := summarizer.SummaryProfile{
		Version:           summarizer.SummaryVersion,
		NarrativeProvider: "genaihub",
		Model:             "claude-example",
	...
	store := &fakeReportStore{
		ids: []string{"needs-fallback"},
		reports: map[string]*summarizer.LoadedCIDReport{
			"needs-fallback": {
				Report: &shared.CIDReport{
					CID:                 "needs-fallback",
					NumAIDs:             10,
					AverageOverallScore: 40,
					Platforms: []shared.PlatformSummary{{
						Name:                "Windows 11",
						NumAIDs:             10,
						AverageOverallScore: 40,
						Compliance:          shared.ComplianceMap{"secure_boot_enabled": 0},
					}},
				},
				SourceSHA256: "hash-needs-fallback",
			},
		},
	}
	failingGenerator := &fakeNarrativeGenerator{
		errByCID: map[string]error{"needs-fallback": errors.New("invalid guidance")},
	}

	stats, err := run(context.Background(), discardLogger(), store, failingGenerator, profile)
	if err != nil {
		t.Fatalf("run returned unexpected error: %v", err)
	}
	if want := (runStats{Total: 1, Fallback: 1}); stats != want {
		t.Fatalf("fallback stats = %+v, want %+v", stats, want)
	}
	metadata := store.writtenMetadata["needs-fallback"]
	if metadata.NarrativeProvider != summarizer.FallbackNarrativeProvider || metadata.Model != "" {
		t.Fatalf("fallback metadata = %+v, want fallback provider without model", metadata)
	}
	if summary := store.writtenSummaries["needs-fallback"]; !strings.Contains(summary, "**Guidance status:**") || !strings.Contains(summary, "`secure_boot_enabled`") {
		t.Fatalf("fallback summary lacks current facts or status:\n%s", summary)
	}

	store.metadata = map[string]summarizer.SummaryMetadata{"needs-fallback": metadata}
	successfulGenerator := &fakeNarrativeGenerator{}
	stats, err = run(context.Background(), discardLogger(), store, successfulGenerator, profile)
	if err != nil {
		t.Fatalf("retry run returned unexpected error: %v", err)
	}
	if want := (runStats{Total: 1, Processed: 1}); stats != want {
		t.Fatalf("retry stats = %+v, want %+v", stats, want)
	}
	assertStringSlices(t, successfulGenerator.summarized, []string{"needs-fallback"})
}
...
type fakeReportStore struct {
	...
	writtenSummaries map[string]string
}
...
func (s *fakeReportStore) WriteSummary(_ context.Context, cid string, summary string, metadata summarizer.SummaryMetadata) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if s.writtenSummaries == nil {
		s.writtenSummaries = make(map[string]string)
	}
	s.writtenSummaries[cid] = summary
