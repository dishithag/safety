...
func (s *ReportStore) LoadCIDReportFromStore(ctx context.Context, cid string) (*shared.CIDReport, error) {
	if err := validateReportID(cid); err != nil {
	...
	report, err := loadCIDReportReader(reader)
	if err != nil {
		return nil, fmt.Errorf("load CID report from store: %w", err)
	}
	if err := validateCIDReportID(report, cid); err != nil {
		return nil, fmt.Errorf("load CID report from store: %w", err)
	}
...
func validateCIDReportID(report *shared.CIDReport, expectedCID string) error {
	if report.CID != expectedCID {
		return fmt.Errorf("CID mismatch: object key requested %q but report payload has %q", expectedCID, report.CID)
	}
	return nil
}

func validateReportID(cid string) error {
	if strings.TrimSpace(cid) == "" {
		return fmt.Errorf("cid is empty")
	}
	if strings.Contains(cid, "/") {
		return fmt.Errorf("cid %q must not contain '/'", cid)
	}
	return nil
}
